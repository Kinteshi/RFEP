...
def load_L2R_file(TRAIN_FILE_NAME, sparse=False):
    ...
    for line in trainFile:
        ...
        if sparse:
            numFeaturesList = (
                re.sub(r":.?[0-9]+.?[0-9]+\s", " ", m.group(3) + " ")).split(" ")
        ...
        for i in featuresList:
            if True:
                if sparse:
                    x_train[iL][int(numFeaturesList[colAllFeat]
                                    ) - 1] = float(i)
                else:
...
def dcg(topN, arrayLabel, dataset):
    ...
    for iPos in range(1, totalDocs):
        ...
        if (iPos < 2):
            ...
            vetDCG[iPos] = vetDCG[iPos - 1] + \
                (getNdcgRelScore(dataset, r) * math.log(2) / math.log(iPos + 1))
    return vetDCG
...
def ____getEvaluationOLD(scoreList, featureFile, metric, resultPrefix, printFold):
    ...
    call(["perl", "/home/daniel/programs/eval_score_LBD.pl",
          featureFile, scoreFile, "L2R.mapGA", "1"])
    startFlag = 0
    iAP = 0
    MAP = 0
...
def getTheModel(ensemble, ntrees, frate, seed, coll, mask):
    seed = seed + 10

    if frate < 0.3:
        frate = 0.3
    if ensemble == 1:
        clf = Forest(n_estimators=ntrees, max_features=frate, max_leaf_nodes=100, min_samples_leaf=1,
    ...
    if ensemble == 10:
        if "200" in coll:
            clf = DecisionTreeRegressor(
                random_state=seed, min_samples_leaf=30, min_samples_split=50, max_depth=3)
        else:
            clf = DecisionTreeRegressor(
                random_state=seed, min_samples_leaf=50, min_samples_split=100, max_depth=6)
...
def executeRankSVM(Xtrain, yTrain, qTrain, Xtest, yTest, qTest, mask, fold):
    ...
    with open(out2, 'w') as f:
        call(["/home/daniel/programs/svm_rank/svm_rank_classify",
              testFile, modelFile, scoreFile], stdout=f)
